...
# ~3 s CPU-bound command (a DWARF capture of the real benchmark takes minutes
...
# perf_rec TAG -- CMD...: record CMD with the probed event into $OUT/perf_TAG.data
perf_rec(){
...
# ---------------------------------------------------------------- 1. baseline
# 10 processes x 3 values: enough for a mean and standard deviation, and keeps
# the run inside one VM session. Override with PYPERF_OPTS for more.
...
venv/bin/py-spy record --rate 500 -f raw -o "$OUT/pyspy_base.folded" -- \
    $PY "$BASE" --worker --loops "${PROF_LOOPS:-4}" -n 2 -w 0 >/dev/null 2>&1 || true
...
if [ -d venv-dbg ]; then
  perf_guide opt_dbg -- venv-dbg/bin/python "$OPT" --worker --loops "${PROF_LOOPS:-4}" -n 2 -w 0
fi
venv/bin/py-spy record --rate 500 -f raw -o "$OUT/pyspy_opt.folded" -- \
    $PY "$OPT" --worker --loops "${PROF_LOOPS:-4}" -n 2 -w 0 >/dev/null 2>&1 || true
# py-spy 0.4 sometimes exits 1 ("No child process") after writing a complete file: judge it by the file
...
# for the accelerator rests on them). cProfile traces every call and inflates
# absolute times, so it is used for the shares only, never for wall-clock figures.
...
# ---------------------------------------------------------------- 3c. attribution
# Two measurements behind section 3 of the report:
#   table_stats.py  what the linear table scan costs on this input
#                   (table sizes, entries compared per symbol, bit peeks)
#   ablation.py     what each separable optimization is worth on its own
log "table-scan statistics for the shipped decoder"
$PY scripts/table_stats.py "$OUT/table_stats.txt" || echo "table_stats failed"
log "per-optimization ablation (each change reverted in turn, md5-checked)"
$PY scripts/ablation.py "${ABLATION_REPS:-5}" "$OUT/ablation.txt" || echo "ablation failed"
...
# 1.30 s for the same 31.4 billion instructions). perf's "CPUs utilized" shows this.
for v in base opt; do
