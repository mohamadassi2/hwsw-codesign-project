...
reaches the corners the six directed sets are built for: every code length, a
clean end-of-input, an unmatchable code, a table index out of range, a stream
that stops mid-code, and a buffer that drains to empty. `hw/tb/MUTATIONS.md` records the whole table.
