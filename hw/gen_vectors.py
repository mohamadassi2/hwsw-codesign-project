...
block:
  ...
  vectors/tables.txt    one line per programmed entry:
                          R <tsel> <L> <limit> <base>      (per code length)
                          S <tsel> <idx> <sym>             (symbol table)
  vectors/stream.hex    the compressed bytes starting at the byte that holds
                        the first Huffman symbol, one byte per line (hex)
  vectors/meta.txt      skip_bits=<leading bits of the first byte to drop>
                        nsym=<number of expected symbols>
  vectors/expected.txt  one line per decoded symbol: <tsel> <sym> <len>
...
def find_logged(self, field, reversed=True):
    ...
    if tid not in state['tsel_of']:
        tsel = len(state['tsel_of']); state['tsel_of'][tid] = tsel
...
with open(os.path.join(VEC, 'meta.txt'), 'w') as fo:
    fo.write(f'skip_bits={skip_bits}\nnsym={len(expected)}\nnbytes={end_byte-start_byte}\n'
             f'ntables={max(t for t,_,_ in expected)+1}\nmaxbits={max(l for _,_,l in expected)}\n')
...
print(f"blocks covered: {NBLOCKS}; tables: {ntab}; symbols: {len(expected)}; "
      ...
      f"code lengths used: {min(l for _,_,l in expected)}..{max(l for _,_,l in expected)}; "
